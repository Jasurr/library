export function query(selector: string, root?: HTMLElement): Element {
    ...
    function __query(
        ...
        if (!element && document.querySelector(selector)) {
            match = document.querySelector(selector) as Element
        }
        ...
        while (walk.nextNode()) {
            if ((walk.currentNode as Element).querySelector(selector)) {
                match = (walk.currentNode as Element).querySelector(selector) as Element;
            }
    ...
    return __query(selector, root as HTMLElement);
}
...
export function queryAll(selector: string, root?: HTMLElement): Element[] {
    ...
    return __queryAll(selector, [], r as HTMLElement);
}